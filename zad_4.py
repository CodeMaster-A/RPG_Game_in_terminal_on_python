...
class Player(ABC):
    ...
    def learn_spell(self, spell):
        if spell.name not in self.spells_book:
            self.spells_book[spell.name] = spell
            print(f"{self.name} выучил новую способность: {spell.name}")
        else:
...
class Monster:
    ...
    def respown(self):
        ...
        print('Монстр воскрешен!')
        return f"Монстр воскрешен! {self.health}|{self.resource}"
...
class Spell:
    ...
    def use(self, damage, player: Player, target: Monster):
        if player.resourse >= self.size_res:
...
m1 = Monster('Monster')
>>>>>>> 9c647f83ab25950feb12b04a261f32c5a30af94c
